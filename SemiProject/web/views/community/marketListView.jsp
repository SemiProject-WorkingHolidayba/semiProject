...
<script>
  ...
  <form action ="<%=request.getContextPath()%>/marketSearch.bo">
...
function search(){
      ...
      <div class="pagingArea" style="margin-left: 550px; margin-top:30px;">
         <!-- 맨 처음으로(<<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage=1 %>&categoryName=벼룩시장'"> << </button>
         <!-- 이전 페이지로(<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage-1 %>&categoryName=벼룩시장'"> < </button>
         <!-- 10개의 페이지 목록 -->
         <% for(int p = startPage ; p <= endPage ; p ++) {%>
            ...
            <%}else{ %>
               <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=p %>&categoryName=벼룩시장'"><%=p %></button>
            <%} %>
         <% } %>
         ...
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage+1 %>&categoryName=벼룩시장'"> > </button>
         <!-- 맨 끝으로(>>) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=maxPage %>&categoryName=벼룩시장'"> >> </button>
      </div>
  <script>
   // 게시판 상세보기 기능을 만들어 보자
   $(function(){
