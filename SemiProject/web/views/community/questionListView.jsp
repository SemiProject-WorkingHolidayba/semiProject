...
<title>Insert title here</title>
 ...
 <%@ include file="../common/menubar.jsp" %>
  ...
  <img src = "<%=request.getContextPath() %>/write.png" style= "width:60px; height:30px; margin-left:700px; margin-top:40px; position:absolute; cursor:pointer;" onclick="location.href='views/community/communityInsertForm.jsp'   ">
  
  <form action ="<%=request.getContextPath()%>/questionSearch.bo">
...
function search(){
   ...
   <div class="pagingArea" style="margin-left: 550px; margin-top:30px;">
         <!-- 맨 처음으로(<<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage=1 %>&categoryName=질문게시판'"> << </button>
         <!-- 이전 페이지로(<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage-1 %>&categoryName=질문게시판'"> < </button>
         <!-- 10개의 페이지 목록 -->
         <% for(int p = startPage ; p <= endPage ; p ++) {%>
            ...
            <%}else{ %>
               <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=p %>&categoryName=질문게시판'"><%=p %></button>
            <%} %>
         <% } %>
         ...
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage+1 %>&categoryName=질문게시판'"> > </button>
         <!-- 맨 끝으로(>>) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=maxPage %>&categoryName=질문게시판'"> >> </button>
      </div>
  <script>
   // 게시판 상세보기 기능을 만들어 보자
   $(function(){
