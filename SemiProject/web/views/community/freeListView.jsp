...
<title>Insert title here</title>
 ...
 <%@ include file="../common/menubar.jsp" %>
  ...
  function goMarket(category){
   ...
   <br>
   <br>
      <h1 Style="color:#ADD4D9; font-weight:600; font-size:50px;">자유게시판 </b> </h1>
  ...
  <form action ="<%=request.getContextPath()%>/freesearch.bo">
 ...
 <select name="selectbox" id = "category" style = "margin-bottom:20px; height:30px; ">
  ...
  <a class = "btn btn-info"style= "height:30px;"  >검색</a>
  </form>
      ...
      <div class="pagingArea" style="margin-left: 550px; margin-top:30px;">
         <!-- 맨 처음으로(<<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage=1 %>&categoryName=자유게시판'"> << </button>
         <!-- 이전 페이지로(<) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage-1 %>&categoryName=자유게시판'"> < </button>
         <!-- 10개의 페이지 목록 -->
         <% for(int p = startPage ; p <= endPage ; p ++) {%>
            ...
            <%}else{ %>
               <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=p %>&categoryName=자유게시판'"><%=p %></button>
            <%} %>
         <% } %>
         ...
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=currentPage+1 %>&categoryName=자유게시판'"> > </button>
         <!-- 맨 끝으로(>>) -->
         <button onclick="location.href='<%=request.getContextPath() %>/list.bo?currentPage=<%=maxPage %>&categoryName=자유게시판'"> >> </button>
      </div>
  <script>
   // 게시판 상세보기 기능을 만들어 보자
   $(function(){
