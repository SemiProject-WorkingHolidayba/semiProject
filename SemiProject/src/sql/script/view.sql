...
CREATE OR REPLACE VIEW MYCLIST
AS
...
AS
SELECT ROWNUM RNUM ,PERIOD,TITLE,JOBAPPLYNO,USERNAME,EMAIL,USERNO			
FROM (SELECT J.PERIOD,J.TITLE,A.JOBAPPLYNO,M.USERNAME,M.EMAIL,J.USERNO
      FROM JOBSEARCH J
      JOIN JOBAPPLICATION A ON (A.JOBNO = J.JOBNO) 
      JOIN MEMBER M ON (M.USERNO = A.USERNO)
      ORDER BY 1 DESC);
